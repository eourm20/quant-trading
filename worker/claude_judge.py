...
[OBV] 후행 지표 — 가격 반등 후 OBV가 따라옴. 상승=매수세 우위, 하락=매도세 우위이나 홀드 단독 근거 사용 금지. OBV 방향과 가격 방향 불일치=다이버전스(보조 참고)
...
[돌파] 강한 거래량 동반 돌파 = 추가 움직임 기대. 거래량 없는 돌파 = 신뢰도 낮추는 참고 신호 (홀드 단독 결정 금지 — 가격 구조·R/R로 최종 판단)
...
목표가 도달 시 기본 50% 부분 익절. 전량 익절은 RSI≥75 + 거래량 급증 + 명확 역전 신호 동시 충족 시만. 잔량은 손절선 매도가 -3%로 상향 후 보유
...
- 거래량은 신뢰도 보조 지표다: 1.0배+ = 확신 강함, 0.5~1.0배 = 중립(가격 구조·R/R로 종합 판단), 0.5배 미만 = 주의(속임수 가능). 거래량 단독으로 홀드 결정 금지.
- OBV는 후행 지표다: 가격이 먼저 반등한 후 OBV가 따라오므로, OBV 하락을 홀드의 단독 근거로 사용 금지. 가격 구조(MA 배열·피보나치·패턴)가 우선이다.
- 주요 판단 순서: ① 가격 구조(MA·피보나치·패턴 위치) ② R/R 비율 ③ RSI·스토캐스틱 등 오실레이터 ④ 거래량·OBV(보조 확인)
...
**exit** (매도·익절·손절 검토)
- 목표가 도달 → 기본값은 **부분 익절(50%)**, 전량 익절은 아래 조건 모두 충족 시만:
  ① RSI ≥ 75 (극단 과매수) AND ② 거래량 급증(1.5배+) 동반 AND ③ 명확한 역전 신호(음봉·MA 이탈·베어리시 엔걸핑 등)
  → 위 조건 미충족 시: [매도] 판정에 "부분 익절 50%, 잔량 보유, 손절선 매도가 -3%로 상향" 명시
- 손절가 도달 → [매도] 원칙. 손절가 근접 시 홀드 유혹 금지
- 과매수 단독/약한 약세 신호는 전량매도 금지. 부분매도(30~50%) 또는 홀드 우선
- 판단: [매도] or [홀드]
