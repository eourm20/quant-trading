...
이 브랜치(`main`)는 Claude를 직접 호출하므로 재개하려면 `ANTHROPIC_API_KEY` 갱신이 필수이며, `use_agent_mode` 설정은 이 브랜치에 존재하지 않습니다. 다만 현재 서버에 체크아웃된 브랜치는 `feature/agent-mode`이므로, 그대로 재개하면 에이전트 경로로 동작합니다.
